...
exports.emailVerification = async (req, res) => {
  ...
  // Sending OTP
  await sendEmailVerificationOTP(email).then(result => {
...
async function sendPasswordResetOTP(email) {
  ...
  password_resets = await new Promise((resolve, reject) => {
    db.query('SELECT * FROM password_resets WHERE email = ?', [email], async (err, results) => {
      if (err) {
        reject ({ status: 500, code:"ERR-FAIL", message: 'Could not send email' });
        return;
      }      
      resolve(results); // Storing the results in the password_resets array
    });
  });

  if (password_resets.length > 0) {
    // Check if the last email was sent less than 1 minutes ago
    if (Date.now() - password_resets[0].modified_at < 60000) {
      return { status: 429, code: "ERR-TOO-MANY-REQUESTS", message: "Too many requests" };
    }
    ...
    let out = await new Promise((resolve, reject) => {
      db.query('UPDATE password_resets SET code = ?, modified_at = ? WHERE email = ?', [code, currentTime, email], async (err, results) => {
        if (err) {
          reject({ status: 500, code:"ERR-FAIL", message: 'Could not send email' });            
        }
    ...
    let out = await new Promise((resolve, reject) => {
      db.query('INSERT INTO password_resets (email, code, modified_at) VALUES (?, ?, ?)', [email, code, currentTime], async (err, results) => {
        if (err) {
          reject({ status: 500, code:"ERR-FAIL", message: 'Could not send email' });
        }
...
exports.sendPasswordResetOTP = async (req, res) => {
  ...
  // Sending OTP
  await sendPasswordResetOTP(email).then(result => {
    res.status(result.status).send(result);
  });
}
...
exports.resetPassword = async (req, res) => {
  ...
  // Check if email, code and password are missing
  ...
  // Check if the code is correct
  const password_resets = await new Promise((resolve, reject) => {
    db.query('SELECT * FROM password_resets WHERE email = ?', [email], (err, results) => {
      if (err) {
        reject({ status: 500, code:"ERR-FAIL", message: 'Could not reset password' });
        return;
      }
      resolve(results);
    });
  ...
  if (password_resets.length > 0) {
    if (password_resets[0].code === code) {
      // Delete the entry from the database
      await new Promise((resolve, reject) => {
      ...
      await new Promise((resolve, reject) => {
        db.query('UPDATE users SET password = ? WHERE email = ?', [md5(password), email], (err, results) => {
          if (err) {
            reject({ status: 500, code:"ERR-FAIL", message: 'Could not reset password' });
            return;
          }
  ...
  res.status(200).send({ code: "SUCCESS", message: "Password reset successfully" });
}